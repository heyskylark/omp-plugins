...
Choose the configured general task agent for the full adversarial review, or discovered specialists only when their role, tools, and output contract match the complete template and rubric below. The rubric includes design and maintainability judgments and custom severity/evidence/suggestion output beyond patch-introduced bugs. Bundled `reviewer` therefore cannot fill this full-review assignment, nor can a security-only specialist cover the whole rubric. Overriding a schema does not override an incompatible role's intent. Use existing configured role/model mappings and one independent compatible reviewer per distinct configured model when available; extend or shrink labels A/B/C/D to the actual roster. OMP task dispatch selects `agent`, not a model field. Do not hardcode model IDs, require newly invented role names, change runtime settings, or open configuration PRs as part of this review.
...
Launch the reviewers together in one native `task` batch for genuinely substantial independent review. Shared `context` has `# Goal`, `# Constraints`, and `# Contract`; each `task` has `# Target`, `# Change`, and `# Acceptance`. The contract requires read-only review, evidence-based findings from the full template with its severity, location, finding, evidence, and optional suggestion fields, no child todos, no edits, no formatters/linters/builds/tests, and no recursive interrogate/arena/architect or other orchestrator calls. Select a compatible output contract rather than replacing the full template with a patch-only verdict. The parent owns its todo and `local://` notepad. Reviewer output comes through `agent://<id>`, not a shared mutable report. A valid outcome is an explicit empty findings list.
