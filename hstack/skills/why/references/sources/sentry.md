...
6. **Treat existing Seer analyses as secondary evidence.**

   If an existing AI root-cause analysis is available through a read-only fetch, it can suggest hypotheses, not establish facts. Do not trigger a new analysis job. Actual events, stack traces and timestamps remain primary evidence.
