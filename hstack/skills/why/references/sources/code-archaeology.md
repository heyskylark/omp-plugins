...
Use OMP `glob` to locate ADR Markdown and related test paths, then `grep` with `architecture.decision`, `(TODO|FIXME|HACK|XXX|NOTE)`, or the target symbol on those scoped paths. Read relevant sections with `read`; do not use shell search commands.
