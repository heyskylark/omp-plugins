...
Use the mounted Notion MCP read-only tools. Inspect their schemas and server instructions first. When the available fetch-self capability reports AI search available, use that search; otherwise use the available ordinary search. Tool names below describe operations and must be mapped to the mounted names. Do not create or update pages, comments, databases or agent sessions.

1. **Keyword searches with the available Notion search operation.** Try:
   - The feature name
