...
Use the actual mounted read-only tools and schemas; example vendor operation names are not guaranteed to exist. If a child lacks source access, ask the parent to search; if neither can, record the access gap. Never mutate remote state.
...
Slack MCP tools vary. Check which Slack MCP is available and inspect its tool schema first. Use existing authorized access only. If authentication fails, stop and report the gap. Never join channels, change memberships, mark messages read, or post messages as part of research. If the mounted MCP exposes channel history and replies but no message search, inspect known accessible channels with bounded history/reply reads and report that workspace-wide search was unavailable; do not pretend history browsing was a complete search.
