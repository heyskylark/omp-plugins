...
Code examples for [TypeScript best practices](skill://typescript-best-practices). The underlying principles are language-agnostic. First read and apply [type-system discipline](skill://principle-type-system-discipline); for boundary parsing, also read and apply [boundary discipline](skill://principle-boundary-discipline). Examples are independent alternatives, not one combined module.
...
The random-index example expresses the non-empty domain invariant, but with `noUncheckedIndexedAccess`, TypeScript still treats an arbitrary numeric index as possibly absent. Do not silence that diagnostic with a cast or `!`: explicitly narrow the indexed result or use an API that exposes a proven element. Tuple index `0` is statically known to exist.
...
// Do: store a duration rather than two independently ordered endpoints
type TimeRange = { start: Date; durationMs: number };
```

A plain `number` still permits negative, non-finite, and otherwise invalid durations: validate the domain constraint when constructing a range at its boundary. This representation removes independently ordered endpoints; it does not encode non-negativity in TypeScript. Keep `durationMs` a plain number. Brand it (per Branded types) only if a raw number could be passed where a duration is expected, not by reflex. Pick the representation that makes the bad state unconstructable, then expose the reading you need on top (`pairs.flat()`, a `rangeEnd()` helper).
...
An unchecked `as` can hide a runtime crash; it performs no runtime validation. Establish the claim through validation first, and prefer returning a constructed value when that lets the compiler prove the result.

```ts
// Don't
const user = data as User;

// Do. Validate the complete shape at the boundary.
...
In default arms, assign the remaining union value to a `never`-typed local.
...
`satisfies` checks assignability without replacing the expression's inferred type with the target type. It is not runtime validation or a blanket promise that all literals stay narrow. The following example uses a literal-union target for `theme`.
...
Validate once where data crosses in. Trust types inside. Read and apply [boundary discipline](skill://principle-boundary-discipline).

- **Wire formats** (proto, JSON-RPC): use the existing parser's unknown-field tolerance (for example, `ignoreUnknownFields` where supported) so forward-compatible changes don't break old clients; this is not a universal JSON-RPC option.
...
[Showing lines 1-300 of 319. Use :301 to continue]