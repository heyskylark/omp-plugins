...
- **Route to the right layer.** One-off -> parent-owned `local://` notepad. Recurring fix -> skill or lint rule. Systemic issue -> principle.
- **Close the loop.** Don't just record. Apply now or have the parent create a concrete todo; delegated children report the action instead of maintaining their own todos.

**Anti-patterns:**
- Acknowledging without recording ("I'll keep that in mind" does not persist)
- Recording without routing (a notepad entry about a lint rule that should exist is wasted unless the lint rule gets implemented)
