...
description: "Write or review user-requested docs, RFCs, READMEs, PR descriptions, and commit messages with Diátaxis structure, Google developer style, STE instruction rules, and Global English syntax."
...
## Stay within the requested writing

Apply this standard when the user asks for documentation or prose to be written or reviewed. Reading this skill does not authorize creating project docs, adding a README, rewriting unrelated prose, or publishing anything. Edit only the requested material. A review request produces findings or proposed edits unless the user also requests changes. PR descriptions and commit messages follow the prose rules below, but writing them does not authorize creating a PR, committing, pushing, or merging.

Read and apply `skill://unslop` to every document this skill touches. Use `read` to inspect requested documents and relevant source sections, `grep` or `glob` to locate real names, `edit` for existing files, and `write` only for requested new files. No custom agent, background workflow, or persistent writing service is needed.
...
Don't mix modes: no reference tables inside a tutorial, no tutorial hand-holding inside reference, no arguing inside a how-to. Split and link instead. If splitting requires files outside the user's request, propose the split rather than creating those files.

Style reference: [Diátaxis](https://diataxis.fr/).
...
Style reference: [Google developer documentation style guide](https://developers.google.com/style).
...
Style reference: [ASD Simplified Technical English](https://asd-ste100.org/), Issue 9 (2025). The numbered rules and dictionary live in the specification PDF. The principles above are the transferable core, not the full specification.
...
- No slashes: write "a, b, or both" instead of "a/b" or "and/or". Keep literal paths, commands, and other code identifiers intact.
...
Style reference: John R. Kohl, *The Global English Style Guide* (SAS Press), including its SAS sample chapter and Internet Archive text.

## Voice and repo specifics

- Read and apply `skill://unslop` to every doc this skill touches. That skill owns the slop-pattern catalog: AI vocabulary, filler, hedging, formatting tells.
- PR descriptions and commit messages are writing too. Every layer except Diátaxis applies to them. A PR body is a briefing that a reviewer can read in under a minute. Do not paste agent execution logs, SHA lists, or metric tables. Link them through a destination the reviewer can access, not a session-local artifact URI.
- Product UI strings are not documentation. Use your product's copy guidelines for those.
- Indent code snippets with tabs. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it. If the current task does not permit checking a count, do not invent a result or present it as verified.

## Worked example

This example illustrates editing technique. Its filenames are not claims about the current repository.
